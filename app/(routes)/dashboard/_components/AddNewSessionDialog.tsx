...
{    !suggestedDoctors?    <div>
            ...
            <Textarea onChange={(e)=>setNote(e.target.value)} placeholder='Add Detail here...' className='h-[200px] mt-1'/>
        </div>:
        <div>
...
{       !suggestedDoctors? <Button disabled={!note||loading} onClick={()=>OnClickNext()}>
        ...
        <Button disabled={loading||!selectedDoctor} onClick={()=>onStartConsulation()}> Start Consultation{loading?<Loader2 className='animate-spin'/>:<ArrowRight/>}</Button>}
    </DialogFooter>
  </DialogContent>
</Dialog>
  )
